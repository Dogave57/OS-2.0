...
int virtio_gpu_init(void){
	...
	*(volatile struct virtio_gpu_features_modern*)&gpuInfo.pBaseRegisters->driver_feature = modernFeatures;	
	...
	for (uint64_t i = 0;i<VIRTIO_GPU_MAX_SCANOUT_COUNT;i++){
		...
		createVertexElementListObjectInfo.vertexElementCount = 4;
		...
		*pVertexBuffer = vertexBuffer;
		...
		memcpy((void*)pIndexBuffer, (void*)indexBuffer, sizeof(uint8_t)*36);
		memset((void*)&createResourceInfo, 0, sizeof(struct gpu_create_resource_info));
		createResourceInfo.resourceInfo.resourceType = GPU_RESOURCE_TYPE_3D;
		createResourceInfo.resourceInfo.contextId = subsystemContextId;
		...
		for (uint64_t frameCount = 0;;frameCount++){
			...
			matrix4x4_f32_multiply((float*)translationMatrix, (float*)scaleMatrix, (float*)modelMatrix);
	...
	/*		printf("forward vector: (%f, %f, %f)\r\n", cameraForwardVector.x, cameraForwardVector.y, cameraForwardVector.z);
		...
		gpu_object_delete(gpuId, rasterizerObjectId);
		gpu_object_delete(gpuId, dsaObjectId);	
		gpu_object_delete(gpuId, blendStateListObjectId);
...
//		gpu_resource_delete(gpuId, vertexBufferResourceId);
		...
		transferFromDeviceInfo.boxRect.width = pScanoutInfo->resolution.width;
		transferFromDeviceInfo.boxRect.height = pScanoutInfo->resolution.height;
...
int virtio_gpu_gl_delete_object(struct virtio_gpu_object_desc* pObjectDesc, struct virtio_gpu_response_header* pResponseHeader){
	...
	switch (pObjectDesc->objectType){
		case VIRTIO_GPU_GL_OBJECT_TYPE_SHADER:{
			...
			if (!pShaderObjectDesc){
				printf("shader object descriptor info not linked with shader object descriptor\r\n");
				virtualFreePage((uint64_t)pCommandBuffer, 0);
				return -1;
			}
