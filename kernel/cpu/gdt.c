...
unsigned char* pSafeStack = (unsigned char*)0x0;
unsigned char* pSafeStackTop = (unsigned char*)0x0;
uint64_t safeStackSize = MEM_KB*256;
...
int gdt_tss_init(void){
	if (virtualAlloc((uint64_t*)&pSafeStack, safeStackSize, PTE_RW|PTE_NX, 0, PAGE_TYPE_NORMAL)!=0)
		return -1;
	pSafeStackTop = pSafeStack+safeStackSize;
	memset((void*)&tss, 0, sizeof(struct tss_entry_t));
	tss.rsp0 = (uint64_t)(pSafeStackTop-PAGE_SIZE);
	tss.ist[0] = (uint64_t)(pSafeStackTop-PAGE_SIZE);
	tss.iomap_base = sizeof(struct tss_entry_t);
	gdt_add_long_entry((struct gdt_entry_long_t*)(gdt+3), (uint64_t)&tss, sizeof(struct tss_entry_t)-1, 0x89, 0x00);
	return 0;
}
