...
struct subsystem_desc* pDriveDriverSubsystem = (struct subsystem_desc*)0x0;
...
int drive_register_boot_drive(uint64_t driverId, uint8_t port){
	...
	if (subsystem_get_entry(pDriveSubsystem, 0x0, (uint64_t*)&pDriveDesc)!=0){
		mutex_unlock(&mutex);
		return -1;
	}	
	memset((void*)pDriveDesc, 0, sizeof(struct drive_desc));
	pDriveDesc->driverId = driverId;
	pDriveDesc->driveId = 0;
