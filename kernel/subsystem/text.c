...
int text_subsystem_init(void){
	...
	createResourceInfo.resourceInfo.normal.format = GPU_FORMAT_R8_SNORM;
...
"END\n";
	static const unsigned char fragmentShaderCode[]="FRAG\n"
		...
		"IMM[0] FLT32 { 2.0, 0.0, 0.0, 1.0 }\n"
		"IMM[1] FLT32 { 3.0, 127.0, 0.0, 0.75 }\n"
		"DCL TEMP[0]\n"
		"DCL TEMP[1]\n"
		"DCL TEMP[2]\n"
		"DCL TEMP[3]\n"
		...
		"MUL TEMP[1].y, IMM[0].xxxx, TEMP[3].wwww\n"
		"DIV TEMP[1].x, TEMP[1].xxxx, TEMP[1].yyyy\n"
		"MAX TEMP[1].x, TEMP[1].xxxx, IMM[0].zzzz\n"
		"MIN TEMP[1].x, TEMP[1].xxxx, IMM[0].wwww\n"
		"MUL TEMP[1].y, IMM[0].xxxx, TEMP[1].xxxx\n"
	...
	static const unsigned char testFragmentShaderCode[]="GGSL\n"
		...
		"sdfEntry = fdiv(fmul(tex(texCoordIn, samplerState, @2D), imm(127.0)), spreadIn)\n"
		"derivX = fddx(sdfEntry)\n"
		"derivY = fddy(sdfEntry)\n"
		"color = imm(1.0, 0.0, 1.0, 1.0)\n"
		"temp fvec1_32 edge0\n"
		"temp fvec1_32 edge1\n"
		"edge0 = fmul(spreadIn.x, imm(-1.0))\n"
		"edge1 = spreadIn.x\n"
		"temp fvec1_32 smoothValue\n"
		"smoothValue = fdiv(fadd(sdfEntry, imm(1.0)), imm(2.0))\n"
		"smoothValue = min(max(smoothValue, imm(0.0)), imm(1.0))\n"
		"color.w = smoothValue.x\n"
		"colorOut = color\n";	
	...
	samplerState.wrap_s = GPU_TEXTURE_WRAP_CLAMP_TO_BORDER;
	samplerState.wrap_t = GPU_TEXTURE_WRAP_CLAMP_TO_BORDER;
	samplerState.wrap_r = GPU_TEXTURE_WRAP_CLAMP_TO_BORDER;
	samplerState.magImgFilter = GPU_TEXTURE_FILTER_LINEAR;
	samplerState.minImgFilter = GPU_TEXTURE_FILTER_LINEAR;
	samplerState.minMipFilter = GPU_TEXTURE_FILTER_LINEAR;
	samplerState.minDetailLevel = 4.0f;
	samplerState.maxDetailLevel = 16.0f;
...
KAPI int text_subsystem_font_load(unsigned char* pFontBuffer, uint64_t fontBufferSize, uint64_t* pFontId){
	...
	printf("glyph ID: %d\r\n", glyphId);
	int8_t* pGlyphTextureBuffer = (int8_t*)0x00;
	...
	uint64_t glyphTextureBufferSize = (glyphTextureBufferRect.x*glyphTextureBufferRect.y)*sizeof(int8_t);
	uint8_t glyphSpread = 0x04;
	...
	pScaleVector->z = 0.0f;
	pScaleVector->w = 1.0f;
	struct fvec4_32* pSpreadVector = pScaleVector+0x01;
	pSpreadVector->x = (float)glyphSpread;
	pSpreadVector->y = 0.0f;
	pSpreadVector->z = 0.0f;
	pSpreadVector->w = 1.0f;
