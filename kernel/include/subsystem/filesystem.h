...
typedef int(*fsCreateFunc)(struct fs_mount* pMount, uint16_t* filename, uint64_t fileAttribs);
...
int fs_open(uint64_t mount_id, uint16_t* filename, uint64_t flags, uint64_t* pFileId);
