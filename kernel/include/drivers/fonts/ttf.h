...
typedef int(*TTFBytecodeInstructionFunc)(struct ttf_bytecode_context_desc* pBytecodeContextDesc, uint8_t* pInstructionData, uint8_t argument, struct ttf_status_code* pStatusCode);
#define TTF_SIGNATURE ((uint32_t)0x00010000)

#define TTF_MAX_BYTECODE_CONTEXT_COUNT (8192)
#define TTF_MAX_BYTECODE_FUNCTION_COUNT (65536)
...
#define TTF_STATUS_CODE_SUCCESS (0x00)
#define TTF_STATUS_CODE_UNKNOWN (0x01)
#define TTF_STATUS_CODE_OPERATION_FAILURE (0x02)
#define TTF_STATUS_CODE_OPERATION_UNSUPPORTED (0x03)
...
struct ttf_segment_entry_desc{
	uint16_t activePixelCount;
	uint16_t firstActivePixelOffset;
	uint16_t lastActivePixelOffset;
	uint16_t cornerPixelList[4];
	uint16_t reserved0;
}__attribute__((packed));
...
struct ttf_pixel_entry_desc{
	...
	uint16_t nextActivePixelOffset;
	uint16_t reserved0;
}__attribute__((packed));
struct ttf_bytecode_execution_info{
	uint8_t readOnly;
	struct ttf_point_entry_desc* pPointDescList;
	struct uvec2_32 textureRect;
};
struct ttf_bytecode_execution_state{
	struct ttf_bytecode_execution_info* pExecutionInfo;
	struct fvec2_64 projectionVector;
	struct fvec2_64 freedomVector;
	uint16_t referencePointList[3];
	uint8_t currentRoundingOpcode;
	uint8_t* pCurrentInstruction;
};
struct ttf_bytecode_function_desc{
	uint8_t* pFunction;
}__attribute__((packed));
struct ttf_bytecode_context_desc{
	...
	uint8_t* pStackEntry;
	uint64_t stackSize;
	uint8_t** pReturnStack;
	uint8_t** pReturnStackEntry;
	uint64_t returnStackSize;
	struct ttf_bytecode_function_desc* pFunctionDescList;
	uint64_t functionDescListSize;
	uint8_t* pBytecode;
	uint64_t bytecodeLength;
	struct ttf_bytecode_execution_state executionState;
};
struct ttf_status_code{
	uint16_t code;
	uint16_t subCode;
...
int ttf_glyf_tesselate(struct ttf_font_desc* pFontDesc, uint32_t glyphId, uint8_t* pTextureBuffer, struct uvec2_32 textureBufferRect, uint8_t spread);
...
int ttf_font_glyph_tesselate(struct text_subsystem_font_desc* pSubsystemFontDesc, uint32_t glyphId, int8_t* pTextureBuffer, struct uvec2_32 textureBufferRect, uint8_t spread);
#endif
