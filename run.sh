...
;;
"Darwin")
sudo qemu-system-x86_64 $MACOS_FLAGS -trace "virtio_*" -m 4G -drive if=pflash,format=raw,file=uefi-firmware/OVMF_CODE.fd -drive file=drive.img,if=none,id=nvme_drive,format=raw -device nvme,id=nvme_device,drive=nvme_drive,serial=deadbeef -smp cores=4 -d guest_errors -machine q35
;;
*)
esac
