...
MACOS_FLAGS="-cpu max,+x2apic,+apic -device qemu-xhci,id=xhci -device usb-mouse,bus=xhci.0 -device usb-kbd,bus=xhci.0 -display cocoa,gl=es,zoom-to-fit=on,full-screen=on -device virtio-gpu-gl -serial stdio -d guest_errors,unimp"
